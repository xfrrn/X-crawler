...
async def update_douyin_aweme(aweme_item: Dict):
    ...
    save_content_item = {
        ...
        # 用 sec_uid 做创作者哈希：监控侧存的是 sec_user_id（URL 里那段 MS4wLjAB…），
        # 而 author.sec_uid 与查询用的 sec_user_id 是同一个串，sha256 后能与引擎对监控
        # creator_id 复算的哈希对上，帖子才能归属到正确监控。sec_uid 缺失时兜底 uid。
        "creator_hash": anonymize_user_id(user_info.get("sec_uid") or user_info.get("uid")),  # 创作者匿名哈希(不存原始 id)
